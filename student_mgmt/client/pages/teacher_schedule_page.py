# client/pages/teacher_schedule_page.py
...
class TeacherSchedulePage(QWidget):
    def __init__(self, api: APIClient, user_id: int):
        ...
        # Tab 2: 课程列表视图（显示所有教授的课程，包括没有课程表信息的）
        list_tab = QWidget()
        list_layout = QVBoxLayout(list_tab)
        list_layout.setContentsMargins(0, 0, 0, 0)
        
        # 提示标签
        list_info_label = QLabel("以下显示所有教授的课程，包括未安排具体时间的课程：")
        ...
        # 初始化
        self.init_teacher_info()
        self.init_semesters()  # 先初始化学期下拉框
        # 初始状态显示提示信息，不自动加载数据
        self.show_initial_prompt()

    def init_teacher_info(self):
        """初始化教师信息，获取 teacher_id"""
        try:
            resp = self.api.get("/api/teacher/profile")
            data = resp.json()
            if data.get("status") == "ok":
                teacher_data = data.get("data", {})
                self.teacher_id = teacher_data.get("teacher_id")
    ...
    def init_semesters(self):
        ...
        try:
            # 获取所有学期的数据（不传学期参数）
            params = {"teacher_id": str(self.teacher_id)}
            resp = self.api.get("/api/teacher/schedule", params=params)
            data = resp.json()
    ...
    def refresh(self):
        ...
        if not self.teacher_id:
            QMessageBox.warning(self, "错误", "无法获取教师ID")
            return
    ...
    def fill_course_list(self):
        ...
        for i, course in enumerate(courses_list):
            ...
            classroom = course.get("classroom", "") or "（未分配）"
