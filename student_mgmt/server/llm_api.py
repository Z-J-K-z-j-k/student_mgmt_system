    ...
    try:
        ...
        if role == "admin":
            system_content = (
                ...
                "你拥有整个数据库的统计信息作为背景知识，可以帮助管理员分析学生、教师、课程和成绩数据，"
                "提供数据洞察、趋势分析和决策建议。请用专业、准确的中文回答，可以引用具体的统计数据。"
            )
        elif role == "teacher":
            system_content = (
            ...
            system_content = (
                "你是一个大学生学习规划助手，用简洁、友善的中文提供建议，回答不少于两条具体行动建议。"
            )
...
def _ask_openai(prompt: str, role: Optional[str] = None) -> Optional[str]:
    try:
        ...
        if role == "admin":
            ...
            system_content = "你是一个大学生学习规划助手，用简洁、友善的中文给出建议。"
